...
import type { RoomPlacementItem } from '@/features/master-data/modules/detail-calas/types/detailCalas.types';

interface Props {
  placements: RoomPlacementItem[];
}

const HasilCalasRoomPlacement: React.FC<Props> = ({ placements }) => {
  return (
    <Card className="w-full bg-white shadow-sm border border-gray-100 rounded-xl" bodyClassName="p-6">
      ...
      {placements && placements.length > 0 ? (
        ...
        <div className="text-center py-8 bg-gray-50 rounded-xl border border-dashed border-gray-200">
          <p className="text-sm text-gray-500 font-medium">Belum ada penempatan ruangan untuk Anda.</p>
        </div>
      )}
    </Card>
  );
...
export default HasilCalasRoomPlacement;
