...
import { useCreateQuestionCard } from '../api/questionCard.api';
...
const FormCreateQC = () => {
  ...
  const [kategori, setKategori] = useState<'materi' | 'teknis' | 'kepribadian' | 'motivasi'>('materi');
  const [tingkat, setTingkat] = useState<1 | 2 | 3>(1);
  const [deskripsi, setDeskripsi] = useState('');
  const [namaMateri, setNamaMateri] = useState<string | null>(null);
  
  const createMutation = useCreateQuestionCard();
  // Fetch all materi without tingkat filter
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate(payload, {
      onSuccess: () => {
        toast.success('Question card berhasil ditambahkan');
        setOpenDialog('defaultDialog', false);
      },
      onError: (error: any) => {
        toast.error(error?.response?.data?.message || 'Gagal menambahkan question card');
      }
    });
  };

  return (
    <form id="form-create-qc" onSubmit={handleSubmit} className="space-y-4">
      <div>
