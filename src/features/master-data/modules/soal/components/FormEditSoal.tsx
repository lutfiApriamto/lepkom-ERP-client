...
import { useUpdateSoal, useUploadTempSoalFile, useDeleteTempSoalFile } from '../api/soal.api';
...
const FormEditSoal = () => {
  const { selectedSoal } = useSoalStore();
  
  const [judulSoal, setJudulSoal] = useState(selectedSoal?.judulSoal || '');
  const [materiRef, setMateriRef] = useState<string | null>(selectedSoal?.materiRef?._id || null);
  const [tingkat, setTingkat] = useState<1 | 2 | 3>(selectedSoal?.tingkat as 1 | 2 | 3 || 1);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  
  const [uploadedFileUrl, setUploadedFileUrl] = useState<string | null>(selectedSoal?.file || null);
  const [isTempFile, setIsTempFile] = useState<boolean>(false);

  useEffect(() => {
    if (selectedSoal) {
      setJudulSoal(selectedSoal.judulSoal);
      setMateriRef(selectedSoal.materiRef._id);
      setTingkat(selectedSoal.tingkat as 1 | 2 | 3);
      setUploadedFileUrl(selectedSoal.file || null);
      setIsTempFile(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateMutation.mutateAsync({ id: selectedSoal._id, payload });
  ...
  const isLoading = updateMutation.isPending || uploadTempMutation.isPending || deleteTempMutation.isPending;

  return (
    <form id="form-soal" onSubmit={handleSubmit} className="space-y-5 px-1 py-2">
      <div>
        ...
        {uploadedFileUrl ? (
          <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-blue-100 bg-blue-50/50 rounded-lg gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-gray-800 line-clamp-1">
                  {selectedFile ? selectedFile.name : 'File sudah tersedia'}
                </p>
                {selectedFile ? (
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    Harap hapus terlebih dahulu jika ingin menggantinya.
                  </p>
                )}
              </div>
            </div>
            ...
            >
              ...
              {isTempFile ? 'Hapus File' : 'Hapus File Lama'}
            </Button>
          </div>
        ) : (
          <div 
